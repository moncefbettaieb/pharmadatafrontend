<template>
  <div class="bg-white">
    <div class="mx-auto max-w-7xl px-6 lg:px-8">
      <div class="mx-auto max-w-4xl text-center">
        ...
        <div class="mt-8 flex flex-wrap items-center justify-center gap-x-4">
          <span
            ...
            :class="isAnnual ? 'text-gray-500' : 'text-gray-900'"
            >Mensuel</span
          >
          <button
      ...
      <div
        class="isolate mx-auto mt-16 grid grid-cols-1 gap-y-8 gap-x-6 sm:mt-20 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5"
      >
        <!-- Plan Gratuit -->
        <div
          class="rounded-3xl p-8 ring-1 ring-gray-200 bg-white flex flex-col h-full"
        >
          <div class="flex-1">
            ...
            >
              <li
                v-for="feature in plansStore.plans[0].features"
                :key="feature"
                class="flex gap-x-3"
              >
                <CheckIcon
                  class="h-6 w-5 flex-none text-gray-600"
                  aria-hidden="true"
                />
                {{ feature }}
              </li>
        ...
        <!-- Plan Basic -->
        <div
          class="rounded-3xl p-8 ring-1 ring-gray-200 bg-blue-50 flex flex-col h-full"
        >
          <div class="flex-1">
            <h3 class="text-lg font-semibold leading-8 text-blue-900">Basic</h3>
            <p class="mt-4 text-sm leading-6 text-blue-600">
              1 000 Fiches Produits par mois
            </p>
            <p class="mt-6 flex items-baseline gap-x-1">
              <span class="text-4xl font-bold tracking-tight text-blue-900">
                {{
                  isAnnual
                    ? (plansStore.plans[1].price * 10).toFixed(2) + "€"
                    : plansStore.plans[1].price + "€"
                }}
              </span>
            ...
            <ul
              role="list"
              class="mt-8 space-y-3 text-sm leading-6 text-blue-600"
            >
              <li
                v-for="feature in plansStore.plans[1].features"
                :key="feature"
                class="flex gap-x-3"
              >
                <CheckIcon
        ...
        <!-- Plan Pro (Popular) -->
        <div
          class="rounded-3xl p-8 ring-2 ring-indigo-600 bg-indigo-50 lg:z-10 lg:rounded-b-none relative flex flex-col h-full"
        >
          <div class="absolute -top-4 left-0 right-0 flex justify-center">
            <span
              class="rounded-full bg-indigo-600 px-4 py-1 text-xs font-semibold leading-5 text-white"
          ...
          <div class="flex-1">
            ...
            <p class="mt-4 text-sm leading-6 text-indigo-600">
              5 000 Fiches Produits par mois
            </p>
            <p class="mt-6 flex items-baseline gap-x-1">
              <span class="text-4xl font-bold tracking-tight text-indigo-900">
                {{
                  isAnnual
                    ? (plansStore.plans[2].price * 10).toFixed(2) + "€"
                    : plansStore.plans[2].price + "€"
                }}
              </span>
            ...
            <ul
              role="list"
              class="mt-8 space-y-3 text-sm leading-6 text-indigo-600"
            >
              <li
                v-for="feature in plansStore.plans[2].features"
                :key="feature"
                class="flex gap-x-3"
              >
                <CheckIcon
        ...
        <!-- Plan Enterprise -->
        <div
          class="rounded-3xl p-8 ring-1 ring-gray-200 bg-purple-50 flex flex-col h-full"
        >
          <div class="flex-1">
            <h3 class="text-lg font-semibold leading-8 text-purple-900">
              Enterprise
            </h3>
            <p class="mt-4 text-sm leading-6 text-purple-600">
              30 000 Fiches Produits par mois
            </p>
            <p class="mt-6 flex items-baseline gap-x-1">
              <span class="text-4xl font-bold tracking-tight text-purple-900">
                {{
                  isAnnual
                    ? (plansStore.plans[3].price * 10).toFixed(2) + "€"
                    : plansStore.plans[3].price + "€"
                }}
              </span>
            ...
            <ul
              role="list"
              class="mt-8 space-y-3 text-sm leading-6 text-purple-600"
            >
              <li
                v-for="feature in plansStore.plans[3].features"
                :key="feature"
                class="flex gap-x-3"
              >
                <CheckIcon
        ...
        <!-- Plan Lifetime -->
        <div
          class="rounded-3xl p-8 ring-1 ring-gray-200 bg-gradient-to-br from-green-50 to-emerald-50 flex flex-col h-full"
        >
          <div class="flex-1">
            <h3 class="text-lg font-semibold leading-8 text-emerald-900">
              Lifetime
            </h3>
            ...
            <p class="mt-6 flex items-baseline gap-x-1">
              <span class="text-4xl font-bold tracking-tight text-emerald-900"
                >999€</span
              >
              <span class="text-sm font-semibold leading-6 text-emerald-600"
                >une fois</span
              >
            </p>
            <ul
              role="list"
              class="mt-8 space-y-3 text-sm leading-6 text-emerald-600"
            >
...
const selectPlan = async (plan) => {
  ...
  try {
    ...
    const createSubscriptionCall = httpsCallable(
      $firebaseFunctions,
      "createSubscription"
    );
    ...
    toast.error(
      error.message ||
        "Une erreur s'est produite lors de la redirection vers le paiement"
    );
  } finally {
    loading.value = false;
  }
...
const selectLifetimePlan = async () => {
  ...
  try {
    ...
    const createLifetimeSessionCall = httpsCallable(
      $firebaseFunctions,
