...
export const createProductPaymentSession = onCall(
  ...
  async (request) => {
    ...
    try {
      ...
      const { items } = request.data as { items: ProductItem[] };
      ...
      // Valider chaque item
      items.forEach((item) => {
        if (!item.title || !item.codereferent) {
          throw new HttpsError(
            "invalid-argument",
            "Données de produit incomplètes"
          );
        }
      });
      ...
      const session = await stripe.checkout.sessions.create({
        ...
        mode: "payment",
        line_items: items.map((item: ProductItem) => ({
          price_data: {
            currency: "eur",
            product_data: {
              name: `Fiche produit - ${item.title}`,
              description: `Code Réferent (CIP ou EAN): ${item.codereferent}`,
              metadata: {
        ...
        metadata: {
          userId,
          productIds: JSON.stringify(items.map((item) => item.codereferent)),
        },
      });
