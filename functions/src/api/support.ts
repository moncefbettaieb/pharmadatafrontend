  ...
  async (request) => {
    ...
    try {
      ...
      if (sessionId) {
        ...
        if (sessionDoc.exists) {
          ...
          console.warn(
            `Session ${sessionId} non trouvée dans la base de données`
          );
        }
      } else {
      ...
      - Créée le: ${
        sessionData?.createdAt?.toDate().toLocaleString() || "non disponible"
      }
    ...
    `;
      ...
      await db.collection("notifications").add({
        ...
        message:
          "Votre message a été envoyé au support. Nous vous répondrons dans les plus brefs délais.",
        type: "info",
        read: false,
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
      });
      ...
      console.error(
        "Erreur détaillée lors de l'envoi de l'email au support:",
        error
      );
      throw new HttpsError(
        "internal",
        "Erreur lors de l'envoi de l'email au support: " +
...
export const sendContactEmail = onCall(
  ...
  async (request) => {
    ...
    if (!subject || !message || !sessionId) {
      throw new HttpsError(
        "invalid-argument",
        "Sujet, message et ID de session requis"
      );
    }
